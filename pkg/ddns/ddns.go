...
func (provider *Provider) UpdateDomain(ctx context.Context) {
	...
	for _, domain := range provider.DDNSProfile.Domains {
		for retries := 0; retries < int(provider.DDNSProfile.MaxRetries); retries++ {
			...
			log.Printf("sysctl>> Updating DNS Record of domain %s: %d/%d", provider.domain, retries+1, provider.DDNSProfile.MaxRetries)
			if err := provider.updateDomain(); err != nil {
				log.Printf("sysctl>> Failed to update DNS record of domain %s: %v", provider.domain, err)
			} else {
				log.Printf("sysctl>> Update DNS record of domain %s succeed", provider.domain)
				break
			}
		}
	}
