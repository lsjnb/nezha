...
func SendNotification(notificationGroupID uint64, desc string, muteLabel *string, ext ...*model.Server) {
	if muteLabel != nil {
		...
		if !flag {
			if Conf.Debug {
				log.Println("sysctl>> Muted repeated notification", desc, muteLabel)
			}
			return
		}
	}
	...
	for _, n := range NotificationList[notificationGroupID] {
		...
		if err := ns.Send(desc); err != nil {
			log.Printf("sysctl>> Sending notification to %s failed: %v", n.Name, err)
		} else {
			log.Printf("sysctl>> Sending notification to %s succeed", n.Name)
		}
	}
}
